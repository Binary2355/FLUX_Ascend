...
class CPPLibBuild(build_clib):
    ...
    def run(self) -> None:
        ...
        flux_ascend_dir = os.path.join(BASE_DIR, build_py.build_lib, build_py.get_package_dir("flux_ascend"))
        if not os.path.exists(flux_ascend_dir):
            os.makedirs(flux_ascend_dir)
        ...
        cmake_args = [
            ...
            "-B",
            self.build_temp,
            f"-DFLUX_ASCEND_PATH={flux_ascend_dir}",
...
class DevelopBuild(develop):
    ...
    def install_for_development(self) -> None:
        ...
        self.run_command("build_py")
        if self.kernel_name:
            print(f"[INFO] Building single kernel: {self.kernel_name}")
...
setup(
    name="flux_ascend",
    version=VERSION,
    keywords="flux_ascend",
    ext_modules=[Extension("flux_ascend._C", sources=[])],
    libraries=[("flux_ascend", {"sources": []})],
    cmdclass={
